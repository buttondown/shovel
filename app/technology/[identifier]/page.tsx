...
export default async function TechnologyPage({ params }: { params: { identifier: string } }) {
  ...
  const technologyCounts = await db
    ...
    .where("domain", "in",
      db.selectFrom("detected_technologies")
        .where("technology", "=", params.identifier)
        .select("domain")
    )
  ...
  return (
    ...
    <div className="p-4 pt-8">
        ...
        {data.map((item) => (
          <div key={item.domain} className="flex flex-col items-center gap-2 p-4 bg-white/10 rounded-lg shadow-md border border-white/15 hover:bg-white/15 hover:border-white/20 transition-colors duration-200">
            <DomainIcon domain={item.domain} />
            <a href={`/${item.domain}`} className="block">{item.domain}</a>
            <div className="text-gray-400 text-xs">{item.creation_date.toLocaleDateString()}</div>
          </div>
        ))}
      ...
      <SectionHeader>Other technologies found on the same domains:</SectionHeader>
      <ul className="grid md:grid-cols-2 lg:grid-cols-4 gap-4 mt-2">
        {technologyCounts.map((item) => (
            <TechnologyPill technology={item.technology} subtitle={item.count} />
        ))}
        </ul>
    </div>
  )
